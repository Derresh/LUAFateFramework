...
if free then 
	...
	if target and slot then
		...
		local conDB = target.properties.conDB.converted
		conDB[slot] = aspect
		target.properties.conDB.value = conDB
	...
	println(token.name," Invokes ", aspect)
	local res, output = macro.call("useFP@lib:Fate")
	print(output)
end

if invoke.frame then intRefresh(invoke.frame) end
